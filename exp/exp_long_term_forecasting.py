...
from models.SARAF import print_gpu_memory_by_module, print_gpu_memory_summary
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _build_model(self):
        ...
        print_gpu_memory_summary(self.args.gpu)
        print("[INFO] Before prepare_dataset:")
        if hasattr(model, 'module'):
            print_gpu_memory_by_module(model.module, self.args.gpu)
        else:
            print_gpu_memory_by_module(model, self.args.gpu)
        
        model.prepare_dataset(train_data, vali_data, test_data)
        
        print("\n[INFO] After prepare_dataset:")
        print_gpu_memory_summary(self.args.gpu)
        if hasattr(model, 'module'):
            print_gpu_memory_by_module(model.module, self.args.gpu)
        else:
    ...
    def vali(self, vali_data, vali_loader, criterion, mode='valid'):
        ...
        with torch.no_grad():
            for i, (index, batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                # encoder - decoder
                if self.args.model == 'SARAF':
                    outputs = self.model(batch_x, batch_x_mark, index, mode=mode)
                else:
...
#         early_stopping = EarlyStopping(patience=self.args.patience, verbose=True)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (index, batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                # encoder - decoder
                if self.args.model == 'SARAF':
                    outputs = self.model(batch_x, batch_x_mark, index, mode='train')
                else:
                    if self.args.use_amp:
                        with torch.cuda.amp.autocast():
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                ...
                # Periodically clear the cache to prevent memory overflow.
                if (i + 1) % 200 == 0:
                    torch.cuda.empty_cache()
            ...
            if vali_loss < best_valid_loss:
                ...
                print(f"\n[INFO] Epoch {epoch + 1}: Saving best model (vali_loss: {vali_loss:.7f})")
                print_gpu_memory_summary(self.args.gpu)
                torch.save(self.model.state_dict(), best_model_path)
                torch.cuda.empty_cache()
                print("[INFO] After cache clear:")
                print_gpu_memory_summary(self.args.gpu)
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (index, batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # encoder - decoder
                if self.args.model == 'SARAF':
                    outputs = self.model(batch_x, batch_x_mark, index, mode='test')
                else:
                    if self.args.use_amp:
                        with torch.cuda.amp.autocast():
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
